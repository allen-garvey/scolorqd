...
void spatial_color_quant(array2d< vector_fixed<double, 3> >& image, array2d< vector_fixed<double, 3> >& filter_weights, vector< vector_fixed<double, 3> >& palette){
    double initial_temperature = 1.0;
    double final_temperature = 0.001;
    int temps_per_level = 3;
    int repeats_per_temp = 1;
        ...
        for(int repeat=0; repeat<repeats_per_temp; repeat++){
            ...
            while(!visit_queue.empty()){
        		...
        		for (unsigned int v=0; v < palette.size(); v++) {
        		    ...
        		    else if (new_val >= 1) new_val = 1 - 1e-10;
    ...
    while (coarse_level > 0) {
        coarse_level--;
        array3d<double>* p_new_coarse_variables = new array3d<double>(image.get_width()  >> coarse_level, image.get_height() >> coarse_level, pixelalette.size());
        zoom_double(*p_coarse_variables, *p_new_coarse_variables);
        delete p_coarse_variables;
        p_coarse_variables = p_new_coarse_variables;
    }

    //find index of best color in palette for every pixel in image
    //code omitted since we are not using it

    //clamp palette colors between 0-1
    for (unsigned int v=0; v<palette.size(); v++) {
    	for (unsigned int k=0; k<3; k++) {
    	    if (palette[v](k) > 1.0) palette[v](k) = 1.0;
    	    else if (palette[v](k) < 0.0) palette[v](k) = 0.0;
    	}
    }
...
int main(int argc, char* argv[]) {
    ...
    //fill palette here with rgb arrays set to random values
    ...
    //Read file here, convert to rgb doubles between 0-1
    ...
    spatial_color_quant(image, *filters[3], palette);
